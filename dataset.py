...
class NewsDataset(Dataset):
    def __init__(self, task, dataset_name, root='..'):
        data = json.load(open('{}/datasets/{}/{}.json'.format(root, task, dataset_name)))
        labels = []
        if task == 'TASK1':
            for item in data:
        ...
        emotion = json.load(open('{}/DATA/{}_{}_emotion_detection.json'.format(root, task, dataset_name)))
        framing = json.load(open('{}/DATA/{}_{}_framing_detection.json'.format(root, task, dataset_name)))
        propaganda = json.load(open('{}/DATA/{}_{}_propaganda_detection.json'.format(root, task, dataset_name)))
        retrieval = json.load(open('{}/DATA/{}_{}_retrieval_entity.json'.format(root, task, dataset_name)))
        graph_info = []
        for index in range(len(data)):
            comment = json.load(open('{}/comment_tree/{}_{}/{}.json'.format(root, task, dataset_name, index)))
            stance = json.load(open('{}/stance/{}_{}/{}.json'.format(root, task, dataset_name, index)))
            relation = json.load(open('{}/generate_prompt/new_relation/{}_{}/{}.json'.
                                      format(root, task, dataset_name, index)))
            row, col = [], []
        ...
        for index in range(len(data)):
            self.data.append({
                'text': data[index][0],
                'emotion': emotion[index][0],
                'framing': framing[index][0],
                'propaganda': propaganda[index][0],
...
def main():
    dataset = NewsDataset('TASK3', 'semeval-2023', root='../../..')
    loader = DataLoader(dataset, batch_size=4, collate_fn=my_collate_fn)
    for batch in loader:
        print(batch.keys())
        print(batch['emotion'][0])
